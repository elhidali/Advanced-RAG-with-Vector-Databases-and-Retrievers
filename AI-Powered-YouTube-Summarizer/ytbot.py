...
def get_embedding_model():
    LOCAL_EMBED_MODEL = os.path.join(os.path.dirname(os.path.dirname(os.path.abspath(__file__))), "bge-small-en-v1.5")
...
def create_summary_chain(llm, prompt):
    """
    Create a chain using LCEL for generating summaries.
...
def create_qa_chain(llm, prompt_template, verbose=True):
    """
    Create a chain using LCEL for question answering.
    ...
    Returns:
        Chain: An instantiated LCEL chain ready for question answering.
    """
...
def generate_answer(question, faiss_index, qa_chain, k=7):
    ...
    # Generate answer using the QA LCEL chain
    answer = qa_chain.invoke({"context": formatted_context, "question": question})
...
def summarize_video(video_url):
    ...
    if processed_transcript:
        # Step 1: Initialize OpenRouter LLM for summarization
        llm = get_llm()
...
def answer_question(video_url, user_question):
    ...
    if processed_transcript and user_question:
        ...
        # Step 2: Initialize OpenRouter LLM for Q&A
        llm = get_llm()
 
        # Step 3: Create FAISS index for transcript chunks
